...
sleep 5
vault operator init -key-shares=1 -key-threshold=1 > ../vault-key.txt

sleep 2

vault operator unseal $(grep 'Key 1:' ../vault-key.txt | awk '{print $NF}')
cp ../vault-key.txt ./vault-init.txt
